        ...
        */}
        <div className="lg:w-[340px] flex-shrink-0 bg-white border-r border-gray-200 flex flex-col justify-center items-center p-6 relative z-20">
            ...
            <button
              ...
              className="lg:hidden absolute top-6 right-6 text-gray-500 hover:text-[#8C41D2] p-2"
            >
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          {/* ROW 1: Purple Info Bar */}
          <div className="bg-[#8C41D2] text-white h-[50px] px-16 flex justify-between items-center text-[13px] font-medium w-full">
            <div className="flex items-center gap-3 group cursor-pointer">
              <span>Service Areas Covered</span>
              <div className="bg-[#FC7801] h-[22px] w-[34px] flex items-center justify-center group-hover:bg-[#D96600] transition-colors shadow-sm">
                <ArrowRight className="h-3.5 w-3.5 text-white" />
              </div>
            </div>
            <div className="hidden sm:block text-right ml-4 tracking-tight opacity-95">
              5 Cornell Avenue, Somerdale, NJ 08083
            </div>
          </div>

          {/* ROW 2: Orange Action Bar */}
          <div className="bg-[#FC7801] text-white h-full  px-16 flex items-center justify-between w-full">
             {/* Phone Section */}
             <div className="flex items-center gap-5">
               ...
               <div className="flex flex-col">
                 ...
                 <div className="text-[17px] font-bold leading-tight mt-0.5">
                   (732) 822-7353 <span className="font-normal text-[15px] opacity-80 mx-1">or</span> (732) 357-5702
                 </div>
               </div>
             </div>
             ...
             <div className="flex items-stretch h-full w-full">
              {navItems.map((item) => (
                <Link
                  key={item.path}
                  to={item.path}
                  className={`flex-1 flex items-center justify-center text-[13px] font-bold tracking-wider px-2 py-5 border-r border-gray-100 hover:text-[#FC7801] hover:bg-gray-50 transition-all relative group h-[60px]`}
                >
                  {/* Active Indicator Line - Positioned absolutely at bottom */}
                  {isActive(item.path) && (
                    <div className="absolute bottom-0 left-0 w-full h-[5px] bg-[#FC7801]"></div>
                  )}
                  
                  <span className={`${isActive(item.path) ? 'text-[#8C41D2]' : 'text-gray-600'}`}>
                    {item.label}
                  </span>
                </Link>
              ))}
            ...
            {navItems.map((item) => (
              <Link
                ...
                className={`px-6 py-4 text-sm font-bold uppercase tracking-wider ${
                  isActive(item.path)
                    ? 'bg-purple-50 text-[#8C41D2] border-l-4 border-[#FC7801]'
                    : 'text-gray-600 hover:text-[#8C41D2] hover:bg-gray-50'
                }`}
              >
                {item.label}
              </Link>
